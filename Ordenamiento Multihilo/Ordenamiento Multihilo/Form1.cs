   ...
   
{
    ...
    public partial class Form1 : Form
    {
        ...
        {
            string filePath = "Reporte_Ordenamiento.docx";
            string burbujaTiempo = cancelRequested ? $"Detenido ({relojBurbuja.ElapsedMilliseconds} ms)" : $"{relojBurbuja.ElapsedMilliseconds} ms";
            string quickTiempo = cancelRequested ? $"Detenido ({relojQuick.ElapsedMilliseconds} ms)" : $"{relojQuick.ElapsedMilliseconds} ms";
            string selectionTiempo = cancelRequested ? $"Detenido ({relojSelection.ElapsedMilliseconds} ms)" : $"{relojSelection.ElapsedMilliseconds} ms";
            string mergeTiempo = cancelRequested ? $"Detenido ({relojMerge.ElapsedMilliseconds} ms)" : $"{relojMerge.ElapsedMilliseconds} ms";
            string contenido = "--- Reporte de Ordenamiento Multihilo ---\n\n";
            contenido += $"Algoritmo Bubble Sort:\tTiempo: {burbujaTiempo}\n";
            contenido += $"Algoritmo QuickSort:\tTiempo: {quickTiempo}\n";
            contenido += $"Algoritmo Selection Sort:\tTiempo: {selectionTiempo}\n";
            contenido += $"Algoritmo Merge Sort:\tTiempo: {mergeTiempo}\n";

            try
            {
                
                System.IO.File.WriteAllText(filePath, contenido);
                MessageBox.Show($"Reporte guardado en: {System.IO.Path.GetFullPath(filePath)}", "Reporte Generado");
            }
            catch (Exception ex)
            {
        ...
        private void chartTiempos_Click(object sender, EventArgs e)
        {
